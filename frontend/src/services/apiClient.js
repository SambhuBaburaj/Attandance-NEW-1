...
// https://attandance-new-1.onrender.com/api
const API_BASE_URL = __DEV__
  ? "http://localhost:3001/api" // Development - use your computer's IP
  : "https://attandance-new-1.onrender.com/api"; // Production deployment on Render
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (__DEV__ && error.config?.metadata) {
      ...
      console.error(
        `❌ ${error.config.metadata.method} ${error.config.metadata.url} - ${
...
export const isNetworkError = (error) => {
  return (
    error.isNetworkError || error.code === "NETWORK_ERROR" || !error.response
  );
};
